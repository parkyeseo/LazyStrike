...
class ViTWithStabilityScore(nn.Module):
    ...
    def aggregate(self, patches: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor | None]:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        patches = self.forward_features(x)
        cls, _scores = self.aggregate(patches)
        return self.head(cls)

    @torch.no_grad()
    def forward_with_scores(self, x: torch.Tensor):
        patches = self.forward_features(x)
        cls, scores = self.aggregate(patches)
